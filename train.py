...
def train():
    ...
    for i in range(10000):
        ...
        proj_image = torchvision.io.read_image("balloons/out.png").type(torch.float32)
        # reshape and normalize tensor
        # proj_image = (proj_image.reshape(1,3,256,256).to(device) / 127.5) - 1
        proj_image = proj_image.reshape(1,3,256,256).to(device)
        processed_image = gmodel(proj_image)
        # convert tensor to image to save
        # processed_image = ((processed_image.detach() + 1)*127.5).type(torch.uint8).reshape(3,256,256)
